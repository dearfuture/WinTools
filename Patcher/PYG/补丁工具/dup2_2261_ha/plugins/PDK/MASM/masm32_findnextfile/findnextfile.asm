...
.data
pinfo				PLUGIN_INFO <DUP2_PLUGIN_VERSION,"com.d2k2.findnextfile","[查找文件]","findnextfile.d2p">
...
DialogPlugin proc uses ebx esi edi hwnd,message,wparam,lparam
	...
	.if eax==WM_INITDIALOG
		...
		;---load window position (use the unique id)---
      		...
      		;---tooltip---
      		fn AddToolTip,hwnd,DLG_BASEFOLDER,chr$("在文件夹中搜索。示例:",13,10,"\subfolder",13,10,"%WINDIR%\system32")
      		fn AddToolTip,hwnd,DLG_FILENAME,chr$("文件过滤可以包含通配符。示例:",13,10,"*.*",13,10,"*.exe")
      		fn AddToolTip,hwnd,DLG_ENV_VAR,"请在这里输入你想要用于保存文件路径的环境变量的名称!"
	...
	.elseif eax==WM_COMMAND
		...
		.elseif ax==BTN_SAVE
			...
			.if eax==0
				fn MessageBox,hDialogPlugin,"请输入一个文件过滤条件",0,MB_ICONEXCLAMATION
				jmp @no_save
			.endif
		
			fn GetDlgItemText,hDialogPlugin,DLG_ENV_VAR,addr local_buffer,sizeof local_buffer
			.if eax==0
				fn MessageBox,hDialogPlugin,"请输入环境变量的名称",0,MB_ICONEXCLAMATION
				jmp @no_save
			.endif
